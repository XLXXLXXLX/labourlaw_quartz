(() => {
  const btn = document.getElementById('slide-reveal-btn');
  if (!btn) return;
  btn.onclick = () => {
    ...
    // 按 <hr> 或 <hr/> 分割
    const parts = article.innerHTML.split(/<hr\s*\/?>/i);
    const sections = parts.map(part => {
        ...
        // 删除其中的 <h1> ~ <h5>
        for (let i = 1; i <= 5; i++) {
            bq.querySelectorAll(`h${i}`).forEach(h => h.remove());
        }
        });

        return `<section>${doc.body.innerHTML.trim()}</section>`;
    }).join('\n');
    // 创建幻灯片窗口
...
.reveal p {
  ...
  width: 90% !important;
  margin: var(--r-block-margin) 20px !important;
  line-height: 1.3;
}
